...
function capitalA(s) {
    ...
    // Check if string contains only uppercase and lowercase characters
    if (/^[a-zA-Z\s]+$/.test(s) && s.endsWith("A")) {
        // Check if string ends with capital A
        return true;
    } else {
        return false;
    }
...
function ioEmail(email) {
    // Add your logic.
    const pattern = /^[a-zA-Z0-9_%+-]+@[a-zA-Z0-9.-]+\.(io)$/;
...
function imagesSearcher(text) {
    let arr = [];
    // Add your logic.
    // Define pattern to match image names with extensions
    const pattern = /([^\s]+(\.(jpg|jpeg|png)))/gi;

    // Find all matches in the text and add them to the images array
    let match;
