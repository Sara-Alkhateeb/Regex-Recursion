...
function division(number, dividedBy) {
    ...
    const absDivisor = Math.abs(dividedBy);

    return divideRecursive(absNumber, absDivisor, 0) * Math.sign(number) * Math.sign(dividedBy);
}

function divideRecursive(dividend, divisor, quotient) {
    if (dividend < divisor) {
        return quotient;
    }

    return divideRecursive(dividend - divisor, divisor, quotient + 1);
}
